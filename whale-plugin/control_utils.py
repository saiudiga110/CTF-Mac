...
class ControlUtil:
    ...
    def add_container(app, user_id, challenge_id, flag, port=0):
        ...
        DockerUtils.add_new_docker_container(app=app, user_id=user_id, challenge_id=challenge_id, flag=flag, uuid_code=uuid_code, port=port)

    @staticmethod
    def remove_container(app, user_id):
        # Fetch container record BEFORE Docker removal to avoid race condition
        container = ControlUtil.get_container(user_id)
        port = container.port if container else 0
